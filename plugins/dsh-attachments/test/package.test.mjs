...
test('package declares a standard installable DSH bundle', async () => {
  ...
  assert.equal(manifest.name, 'dsh-attachment')
  assert.equal(manifest.icon, './assets/markdown/attachment.svg')
  ...
  assert.match(patch, /id:\s*dsh-attachment/)
  assert.match(patch, /name:\s*dsh-attachment/)
})
...
test('marketplace fixture follows the standalone repository catalog schema', async () => {
  ...
  assert.match(prBody, /https:\/\/raw\.githubusercontent\.com\/WJZ-P\/dsh-attachments\/main\/assets\/markdown\/attachment\.svg/)
})

test('installation docs use the published npm package', async () => {
  const installSpec = 'dsh-attachment'
  for (const readme of ['README.md', 'README.en.md']) {
    ...
    assert.doesNotMatch(body, /dsh plugin --profile web add github:WJZ-P\/dsh-attachments/)
  }
})
