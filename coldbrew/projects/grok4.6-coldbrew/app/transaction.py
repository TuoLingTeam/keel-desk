#!/usr/bin/env python3
"""Reversible filesystem transaction for the Grok 4.6 adapter."""
...
def default_home() -> Path:
    configured = os.environ.get("GROK_COLDBREW_HOME")
...
def resolve_layout(home: Path | None = None) -> Layout:
    ...
    return Layout(root, root / "config.json", root / "coldbrew/system-prompt.md", root / "coldbrew/session-template.json", root / ".coldbrew" / "state.json")
...
def verify(home: Path | None = None) -> dict:
    ...
    checks = {
        ...
        "managed_key": config.get(MANAGED_KEY, {}).get("owner") == "grok4.6-coldbrew",
