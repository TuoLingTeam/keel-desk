#!/usr/bin/env python3
"""Grok 4.6 ColdBrew: preview, deploy, verify and restore a local adapter."""
...
def export_template(destination: Path, profile: str) -> dict:
    ...
    session.write_text(json.dumps({"platform": "Grok 4.6", "profile": profile, "system_prompt_file": prompt.name}, indent=2) + "\n", encoding="utf-8", newline="\n")
...
def _legacy_launch_gui(home: Path | None = None) -> int:
    ...
    window = tk.Tk()
    window.title("Grok 4.6 ColdBrew v1.0.1")
    window.geometry("940x650")
    window.minsize(820, 570)
    bg, panel, text, dim, accent = "#071019", "#0f1b27", "#edf5f8", "#8fa3b3", "#23f5d7" if "Grok" in "Grok 4.6 ColdBrew" else "#397dff"
    ...
    tk.Label(root, text="COLDBREW / Grok 4.6", bg=bg, fg=accent, font=("Consolas", 10, "bold")).pack(anchor="w")
    tk.Label(root, text="Grok 4.6 破甲", bg=bg, fg=text, font=("Segoe UI", 28, "bold")).pack(anchor="w", pady=(4, 2))
    tk.Label(root, text="高速信息流中的可逆会话模板与系统提示词部署器", bg=bg, fg=dim, font=("Segoe UI", 11)).pack(anchor="w")
...
def launch_gui(home: Path | None = None) -> int:
    ...
    return launch_shared_gui(
        title="Grok 4.6 ColdBrew · Workbench",
        model="Grok 4.6",
        subtitle="实时信息流适配器 · 可回溯配置、预览与事务式恢复",
        accent="#23F5D7",
        home=home,
