...
name: eni-radare2
description: 全局自动路由 |
  radare2/r2 命令行二进制分析技能。凡用户要用 r2/radare2 或 rabin2/rasm2/radiff2/r2pipe 分析 PE/ELF/Mach-O/DEX/WASM 等文件，做逆向、反汇编、函数分析、字符串/导入导出检查、patch、二进制对比、十六进制查看或 r2 脚本化，都使用本技能。
---

# radare2 命令行分析

面向 `radare2` CLI 的二进制分析技能。重点是用命令行直接完成侦察、分析、定位、导出与轻量修改，不依赖 GUI。

## 适用场景

当用户有这些意图时优先使用本 skill：
...
```powershell
powershell -File "<skill-root>\scripts\recon.ps1" -TargetPath "C:\path\to\sample.exe"
```

如果需要附带 `r2` 自动分析：

```powershell
powershell -File "<skill-root>\scripts\recon.ps1" -TargetPath "C:\path\to\sample.exe" -RunAnalysis
```
...
```powershell
powershell -File "<skill-root>\scripts\recon.ps1" -TargetPath "sample.exe"
```
...
- 需深度反编译 → `ida-reverse/`
- 发现有趣字符串后需交叉引用 → `ida-reverse/`（IDA 的 xref 更强大）
