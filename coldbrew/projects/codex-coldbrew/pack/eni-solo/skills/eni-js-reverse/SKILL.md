---
name: eni-js-reverse
description: "[DOCUMENTATION ONLY] [仅文档] 全局自动路由 | 配合 js-reverse-mcp 做前端 JavaScript 逆向。覆盖签名链路定位、页面观察取证、运行时采样、本地补环境复现与证据化输出。默认使用当前环境里的 js-reverse_* 工具；需要更强的浏览器/CDP/Hook 能力时联动 jshookmcp。"
---
