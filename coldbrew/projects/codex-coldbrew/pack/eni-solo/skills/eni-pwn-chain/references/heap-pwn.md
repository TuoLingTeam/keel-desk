...
tcache 是 per-thread cache，每个 size class 一条链表，单链表（只有 fd）。double free 检查在 2.29 之前只看链表头是不是自己，不做遍历。
...
add(13, 0x30, p64(libc.sym['system']))  # 取出的就是 __free_hook 地址，写 system
...
2. 计算混淆值：fake_fd_obf = (chunk_addr >> 12) ^ target
3. 写进去
```
...
```text
关键点：
1. fastbin 单链表（只有 fd），无 size 检查，除了 chunk size 必须匹配
2. 2.27 后 tcache 优先，fastbin 只有 tcache 满了才用
3. 需要伪造一个看起来像 chunk 的内存（size 字段 = 真实 chunk size，± 一些）
```
...
Step 4: getshell
  ...
  - 2.34+: setcontext + 53 gadget → heap 上的 rop chain → execve
```
...
```text
目标：程序调用 puts/printf 时，最终走到 _IO_file_xsputn → _IO_OVERFLOW → 调用 vtable
劫持：
  1. 覆盖 _IO_2_1_stderr_ 的 vtable 指针指向伪造 vtable
