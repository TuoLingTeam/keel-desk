...
原理：
  ...
  - execve 一个不认识 magic 的文件时，内核以 root 调 modprobe_path
...
**modprobe_path 地址来源**：vmlinux 符号，或 /proc/kallsyms（如果 kptr_restrict=0）。
...
如果想跳回用户态 shellcode（学习目的），可以 ROP 关 cr4 的 bit:
...
- **CONFIG_RANDOM_KSTACK_OFFSET / RANDOMIZE_KSTACK_OFFSET_DEFAULT** 让内核栈基址每次 syscall 随机偏移 0-1023，影响所有依赖固定栈偏移的利用
