...
description: "[DOCUMENTATION ONLY] [仅文档] Cross-version symbol migration and binary differencing workflow. Symbol migration across binary versions plus binary-diff methodology."
---
...
1. **内核/驱动缺 PDB** — 手里有旧版 ntoskrnl.exe 的符号，新版 PDB 被微软下架，需要用旧版符号推导新版非导出函数地址
2. **程序更新后不想重逆** — 之前逆向过某个程序，它更新了，用旧版成果批量迁移符号
3. **保护机制更新** — 旧版有完整逆向结果，新版要快速定位同一函数的新偏移
...
| **本 skill（LLM 批量比对）** | **约 1 元** | **约 10 秒/函数** | **高** |
...
- prompt 固定模板、程序化填充，输入输出格式确定，可程序化解析
- LLM 只承担"读两段代码、找对应关系"这一个动作
- 时间与 token 成本都压到极低
...
Step 2: 批量导出
  - 旧版导出：锚点函数的反汇编 + 伪代码（含符号名）
  - 新版导出：同一锚点函数的反汇编 + 伪代码（无符号名）
