...
> 面向 APK .so、ELF 二进制与控制流平坦化场景的 OLLVM 脱密工作流。
...
| 遇到 BR 混淆（间接分支） | **DeObfBR** | 数据段设只读 | Goron/Arkari 风格的 BR 混淆可被数据段只读简单对抗 |
...
| Hodur(PlugX) 样本没有 switch 分发器 | 嵌套 while(1) + jnz state | 用 d810-ng **HodurUnflattener**，别用普通 Unflattener |
