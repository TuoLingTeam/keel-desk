...
> Go 编译产物的独特难点：静态链接导致体积庞大、函数数以万计、字符串格式特殊、strip 后符号难恢复。
> 本文覆盖工具链、恢复技巧与实战工作流。
...
- 函数数量 5000-50000+（含整个 runtime 与标准库）
...
即使 strip 了符号，pclntab 通常仍在（Go runtime 依赖它）。

```text
定位方法：
1. 搜 magic bytes: 0xFFFFFFF0 (Go 1.16+) 或 0xFFFFFFFB (Go 1.18+)
2. 用 GoReSym 自动定位
...
Go 字符串不是 C 风格的 null 结尾，而是 `(pointer, length)` 结构：
...
这导致 IDA/Ghidra 默认的字符串识别漏掉大量 Go 字符串。

**解法：**

- `go_parser` 自动识别 Go 字符串
- GoReSym 导出字符串列表
- 手动：定位 `runtime.stringtable` 或沿交叉引用找
...
```text
1. GoReSym -t -d -p binary > symbols.json
   → 即使 strip，pclntab 通常还在
2. GoReSym 失败 → 用 redress
...
4. 对比分析
   → 编译同版本 Go 的 Hello World，binary-diff 对比 runtime 部分
```
...
| 字符串识别不全 | Go 字符串非 null 结尾 | 用 go_parser 或 GoReSym 恢复 |
| 反编译结果难读 | defer/goroutine/interface 让伪代码复杂 | IDA 9.2+ 有改进，或动态分析辅助 |
| Garble 混淆 | 函数名/字符串全随机化 | GoResolver + GoStringUngarbler |
| 版本差异 | 不同 Go 版本的 pclntab 格式不同 | GoReSym 支持 Go 1.2-1.23+ |
| CGo 边界 | Go 和 C 代码混合 | 识别 _cgo_* 函数作分界线 |
