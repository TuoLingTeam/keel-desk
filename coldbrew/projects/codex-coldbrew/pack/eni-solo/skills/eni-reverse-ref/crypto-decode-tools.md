...
> 逆向与 CTF 中频繁遇到加密、编码与哈希数据。本文按场景列出最顺手的工具。
...
| 工具 | Stars | 用途 | 链接 |
|------|-------|------|------|
| **Ciphey** | 18k+ | AI 自动识别并解密（50+ 编码/加密/哈希） | https://github.com/Ciphey/Ciphey |
...
| Hex | 只有 0-9a-f，长度偶数 | `xxd -r -p` / CyberChef |
...
3. Ciphey 失败 → 用 CyberChef Magic 模式
