...
3. 找 file_operations 结构
   搜 register_chrdev / cdev_add / misc_register
...
| `malloc` + 使用 | `call malloc` → 返回值存寄存器 → 后续用该寄存器+偏移访问 |
...
| 源码模式 | 反汇编特征 |
|---------|-----------|
| **虚函数调用** | `mov rax, [rcx]`（取 vtable）→ `call [rax+offset]`（调虚函数） |
| **构造函数** | 分配内存 → 写 vtable 指针 → 初始化成员 |
| **析构函数** | 清理成员 → 可能调 `operator delete` |
| **this 指针** | 第一个参数（rcx/rdi）是对象指针 |
| **继承** | vtable 中含父类虚函数 + 子类覆盖 |
...
```text
1. 找 vtable
   - 搜连续的函数指针数组（在 .rodata 或 .rdata 段）
   - 构造函数中 mov [rcx], offset vtable 写入 vtable 指针

2. 确定类层次
   - vtable 前 -8 偏移处通常是 RTTI 指针（若未 strip）
...
| Clang/LLVM | 类似 GCC 但优化模式不同、`__asan_*`（开了 sanitizer 时） |
